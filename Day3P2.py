...
for x in range(0, int(len(bags)), 3):
    elveUnits.append([bags[x],bags[x+1],bags[x+2]])

for unit in elveUnits:
    foundLetters = ''
    for letter in unit[0]:
        if letter in unit[1] and letter in unit[2] and letter not in foundLetters:
            foundLetters = foundLetters + letter
            if ord(letter) < 97:
                error = error + (ord(letter)-38)
            else:
                error = error + (ord(letter)-96)
